...
function CoffeeShop() {
    // Lấy tableId từ URL tương tự như trong MenuComponent
    ...
    const handleAddToCart = async (drink) => {
        // Nếu cần, bạn có thể lưu tableId trong đối tượng cartItem để xử lý sau này
        const cartItem = {
            drink: { id: drink.id },
            quantity: 1,
            price: drink.price,
    ...
    const getAllFeedbacks = async () => {
        try {
            let fbData = await feedbackService.getAllFeedbacks();
            setFeedbacks(fbData);
        } catch (error) {
        ...
        <>
            ...
            <Container sx={{ mt: 15, pb: 10 }}>
                ...
                <Typography variant="body1" align="center" paragraph fontStyle="italic">
                    Nơi bạn có thể thưởng thức những ly cà phê đậm đà, được pha chế từ những hạt cà phê nguyên chất, cùng
                    những thức uống tinh tế như trà thanh mát và bánh ngọt thơm lừng. Với không gian ấm cúng và phong cách
                    phục vụ chu đáo, <Typography component="span" fontWeight="bold"> DANA Coffee </Typography> là lựa chọn lý
                    tưởng để bạn thư giãn, làm việc hay gặp gỡ bạn bè.
                </Typography>
                {/* Hình ảnh */}
                        ...
                        >
                            {topDrinks.slice(index, index + itemsToShow).map((drink, i) => (
                                ...
                                >
                                    <CardMedia
                                        component="img"
                                        image={drink.imgDrinks || "https://via.placeholder.com/150"}
                    ...
                    <Grid container spacing={2} justifyContent="center">
                        {feedbacks.slice(feedbackIndex, feedbackIndex + feedbackShow).map((fb, i) => (
                            <Grid item xs={12} sm={5} key={i}>
                                <Paper
                                    elevation={2}
                                    sx={{ p: 3, textAlign: "center", borderRadius: 2, backgroundColor: "white", color: "black" }}
                                >
